...
图都在 `docs/figures/`，生成命令在 README 的结果表下面。上台前先确认**口径写在图上**：
`results_main.png` 的标题里有 `300-step budget`，而与官方基线对齐的那组是 100 步预算，
两套数字差得很远（见 `docs/external_reference.md`）。同一张片子里混用两个口径，
是这个项目出过的错。

### 只讲 1 页

> 用 `results_main.png`，标题写"三类动作头，等条件对比"，讲三句：
>
> 1. 同骨干、同 6.80M 可训练参数、同训练预算、同留出划分，唯一的变量是动作头
> 2. 单任务 PickCube：Flow Matching 61±15% > BC 53±19% > Diffusion 13±9%
> 3. 多任务：BC 反超（47/92/41 对 33/95/18），而它推理只要一次前向
>
> 一页的要害是**不要讲成"我的方法最强"**，而要讲成"我量出了三者各自的适用范围"。
> 前者会被追问，后者会被追着往下问。
...
| 页 | 图 | 讲什么 |
|---|---|---|
| 1 | `results_main.png` | 同上 |
| 2 | `pareto_control_rate.png` | 把采样成本换算成**可达控制频率**：任何频率下 FM 的可行域都在 Diffusion 之上。FM 在 2300Hz 仍有 67%，Diffusion 要降到 118Hz 才够到它自己的 33% |
| 3 | `language_ablation_bcx_val10.png` | 喂错任务的指令，看性能掉不掉。StackCube 行 48% 对 12/10/12，指令确实在起作用；PushCube 行 88/90/84/88 纹丝不动 —— **那一格的成功率不能算作语言条件的证据** |

第 3 页最值得保留：它主动划出自己结论的边界，而这是面试里不常见到的东西。

### 备用页（不进正文，被问到再翻）

| 会被问到的 | 图 | 怎么答 |
|---|---|---|
| 「±28% 是不是你没调好」 | `failure_seed_gap.png` | 同配置两个种子 65% 对 13%，差异几乎全落在"抓到了但没送到"（25 → 76）。方差有结构，不是噪声 |
| 「为什么一开始选 Flow Matching」 | `notebooks/w1_multimodal.png` | 双峰目标下 MSE 回归塌到中间那个无效的均值，FM 复原两个模态。接着讲：但我量了这批数据是单峰的，所以 BC 才不输 |
| 「BC mean-pool 那 3% 怎么回事」 | `failure_modes.png` | 59% 的局从未接近方块 —— 目标 token 被 83 个 context token 平均池化稀释成了 1/83 |
| 「三个方法具体差在哪」 | `architecture.png` | 灰框是三条臂逐层相同的部分（编码器、context、4 层骨干、6.80M），彩框才是各自独有的（喂进骨干的 x、损失、采样）。BC 那列没有回环 —— 延迟差距 14.3 ms 对 133.6 ms 就在这里 |
| 「图例里 `d_model=256` 是什么」 | — | 还有一条 `d_model=384` 的加宽对照。加宽后验证 loss 在第 10000 步见底后回升 4.6%，是四条臂里唯一掉头的，直接看到了过拟合 |
